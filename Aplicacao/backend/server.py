...
@app.route('/bateria', methods=['POST'])
def handle_bateria_post():
    try:
        bateria_recebida = request.get_data(as_text=True)
        dados = carregar_dados()

        # Cria ou atualiza o campo de bateria do primeiro pet
        dados["pets"][0]["bateria"] = int(bateria_recebida)
        salvar_dados(dados)
...
@app.route('/ipColeira', methods=['POST'])
def handle_ip_coleira_post():
    print("\nPOST /ipColeira recebido")
    try:
        ip_recebido = request.get_data(as_text=True)
        dados = carregar_dados()

        # Salva o IP recebido no JSON do pet
        dados["pets"][0]["ipColeira"] = ip_recebido
        salvar_dados(dados)

        print(f"IP da coleira atualizado para: {ip_recebido}\n")
        return "OK", 200
    except Exception as e:
        print(f"Erro ao processar POST /ipColeira: {e}\n")
        return "ERRO", 500
...
@app.route('/ipColeira', methods=['GET'])
def get_ip_coleira():
    try:
        dados = carregar_dados()
        ip = dados["pets"][0].get("ipColeira", "")

        if not ip:
            return jsonify({"erro": "IP da coleira não registrado."}), 404
